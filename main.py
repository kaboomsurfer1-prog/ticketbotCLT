...
def build_ticket_embed(member: discord.Member, game_id: str) -> discord.Embed:
    embed = discord.Embed(
        ...
        description=(
            ...
            "Dacă ai dovezi, trimite poze sau video în acest canal.\n\n"
...
class TicketIdModal(discord.ui.Modal, title="Creează Ticket"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        if not isinstance(interaction.user, discord.Member):
            await interaction.response.send_message("❌ Această acțiune poate fi folosită doar pe server.", ephemeral=True)
            return
        ...
        if support_role is None:
            await interaction.response.send_message("❌ Eroare configurare: rolul staff pentru ticket nu a fost găsit.", ephemeral=True)
            return
        if close_role is None:
            await interaction.response.send_message("❌ Eroare configurare: rolul care poate închide ticketul nu a fost găsit.", ephemeral=True)
            return
        ...
        try:
            ...
            await interaction.response.send_message("❌ Botul nu are permisiuni suficiente pentru a crea canale.", ephemeral=True)
        except Exception as e:
            await interaction.response.send_message(f"❌ A apărut o eroare la crearea ticketului: `{e}`", ephemeral=True)
...
class TicketPanelView(discord.ui.View):
    ...
    @discord.ui.button(label="Creează Ticket", style=discord.ButtonStyle.primary, emoji="🎫", custom_id="bot_ticket:create_ticket")
...
class CloseTicketView(discord.ui.View):
    ...
    @discord.ui.button(label="Ticket terminat", style=discord.ButtonStyle.danger, emoji="🔒", custom_id="bot_ticket:close_ticket")
    async def close_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not has_role(interaction.user, TICKET_CLOSE_ROLE_ID):
            await interaction.response.send_message("❌ Nu ai permisiunea să închizi acest ticket.", ephemeral=True)
            return

        channel = interaction.channel
        if not isinstance(channel, discord.TextChannel):
            await interaction.response.send_message("❌ Această comandă poate fi folosită doar într-un canal ticket.", ephemeral=True)
            return
        ...
        await interaction.response.send_message(
            f"🔒 Ticketul a fost închis de {interaction.user.mention}. Canalul se va șterge în `{TICKET_DELETE_DELAY}` secunde."
        )
...
async def ticket_status(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(f"✅ Ticketuri deschise: `{len(active_tickets)}`", ephemeral=True)
